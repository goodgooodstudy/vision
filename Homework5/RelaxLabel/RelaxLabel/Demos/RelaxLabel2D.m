...
if nargin<1 || isempty(A) 
    error('Invalid entry for the first input argument')
end

% Check the format of the main array
siz_A=size(A); D=size(A,3);
if sum(siz_A(1:2)<=3)>0 || ~isnumeric(A)
    error('First input argument must be a 2D image or an array of prior probabilities.')
end

% Check the label array
if ~isempty(L)
    siz_L=size(L);
    if numel(siz_L)~=2 || ~isequal(siz_L,siz_A(1:2))
        error('Dimensions of the main and label images do not agree')
    end
    
    if ~islogical(L), L=ceil(L); end
    l=unique(L(:)); % unique labels
    N_lab=numel(l); % total number of labels
    if N_lab==1
        Lout=L; P=1; E=[];
        disp('Label matrix contains only one label')
        return;
    end
    if N_lab>10
        error('Maximum number of labels exceeded. At most 10 labels are allowed')
    end
    ...
    if nargin<3 || isempty(opt)
        error('Incorrect syntax. Label image was not provided.')
    end 
    
    if nargin>=3 && ~isempty(opt) && opt(1)~=2 
        error('Incorrect syntax. Label image was not provided.')
    end
...
if ~ismatrix(opt) || ~isnumeric(opt)
    error('Incorrect entry for 3rd input argument')
end

if numel(opt)<2, opt(2)=1; end
if numel(opt)<3, opt(3)=10; end
if numel(opt)<4, opt(4)=8; end

if ~(opt(1)==1 || opt(1)==2)
    error('opt(1) must be specified as 1 or 2 only')
end

if opt(1)==2 
    chk=sum(A,3);
    if min(chk(:))<(1-10*eps) || max(chk(:))>(1+10*eps)
        error('Probabilities in A must sum to 1 for all pixels')
    end
    clear chk
end

if opt(1)==2 && D~=N_lab
    error('size(A,3) does not equal the unique number of labels in L')
end

if opt(1)==2 && (max(A(:))>1 || min(A(:))<0) 
    error('Probabilities in A cannot be less than 0 or greater than 1')
end

if opt(2)<1 || opt(2)>2
    error('w must be in the range [1 2]')
end
...
if ~(opt(4)==4 || opt(4)==8)
    error('conn can only be equal to 4 or 8')
end
